...
在 [控制语句](controls.md) 和 [表达式](expressions.md)中，我们学习了 C 程序是如何做事情的。现在，我们换一个视角来学习 C 程序操作的内容，即 **值** 和 **数据**。
...
<details>
<summary><strong>TIP: C 程序主要考虑的是值而不是其表示形式</strong></summary>
...
<details>
<summary><strong>NOTE: 位置数字系统</strong></summary>
...
> [!TIP]
> 指示器是 C99 引入的特性，在 [快速入门](getting-started.md) 的第一个示例程序中已经用到了：
>
> ```c
> double A[5] = {
>     [0] = 9.0,
...
C 语言提供了 $5$ 中创建命名常量的方式。在 [快速入门](getting-started.md) 中我们已经介绍 **宏替换**。现在，我们补充上其他四种命名常量
...
> [!TIP]
> 很显然，任何整数类型的最大值都是 $2^p - 1$ 的形式；对于超过 $2^p - 1$ 的无符号值，将触发无符号整数的回绕机制，即 $\bmod 2^p$
...
<details>
<summary><strong>TIP: 补码: 二进制下的关于基数的补数</strong></summary>
