...
这些基本类型我们会在 [基本值和数据] 中详细讲解。基本类型的变量通常只能保存单一数据项；这种只能保存单一数据项的类型称为 **标量类型**。任意一个标量类型都有一个 **真值**(逻辑值)。
